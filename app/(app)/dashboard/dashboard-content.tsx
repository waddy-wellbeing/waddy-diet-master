...
export function DashboardContent({
  ...
  const mealSlots =
    ...
    profile.preferences.meal_structure.length > 0
      ? profile.preferences.meal_structure.map((slot) => ({
          name: slot.name,
          label: slot.label,
        }))
      : [
  ...
  useEffect(() => {
    const saveTodaysPlan = async () => {
      ...
      await saveFullDayPlan({
        ...
        meals: {
          breakfast: breakfast
            ? { recipeId: breakfast.id, servings: breakfast.scale_factor }
            : undefined,
          lunch: lunch
            ? { recipeId: lunch.id, servings: lunch.scale_factor }
            : undefined,
          dinner: dinner
            ? { recipeId: dinner.id, servings: dinner.scale_factor }
            : undefined,
          snacks: snacks
            ? { recipeId: snacks.id, servings: snacks.scale_factor }
            : undefined,
        },
      });
    };
  ...
  const getCurrentRecipe = (mealType: MealName): ScaledRecipe | null => {
    ...
    // Get the plan for the currently selected date from state
    ...
    // Each unplanned day gets DIFFERENT suggestions (deterministic based on date)
    const dateStr = format(selectedDate, "yyyy-MM-dd");
        ...
        {(profile.role === "admin" || profile.role === "moderator") &&
          (() => {
            ...
            const breakfastQuality = getMacroQuality(
              meals.find((m) => m.name === "breakfast")?.recipe || null,
            );
            const lunchQuality = getMacroQuality(
              meals.find((m) => m.name === "lunch")?.recipe || null,
            );
            const dinnerQuality = getMacroQuality(
              meals.find((m) => m.name === "dinner")?.recipe || null,
            );
            const snacksQuality = getMacroQuality(
              meals.find((m) => m.name === "snacks")?.recipe || null,
            );
              ...
              <div className="bg-muted/50 border border-border rounded-lg p-4 text-xs space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <div className="font-mono">
                      {meals.filter((m) => m.isLogged).length} / 4
                    </div>
                  </div>
                  <div>
                ...
                <div className="text-[10px] text-muted-foreground pt-2 border-t border-border">
                  ...
                  {meals.find((m) => m.name === "breakfast")
                    ?.consumedCalories || 0}{" "}
                  | Lunch{" "}
                  {meals.find((m) => m.name === "lunch")?.consumedCalories || 0}{" "}
                  | Dinner{" "}
                  {meals.find((m) => m.name === "dinner")?.consumedCalories ||
                    0}{" "}
                  | Snacks{" "}
                  {meals.find((m) => m.name === "snacks")?.consumedCalories ||
                    0}
                </div>
              </div>
            );
